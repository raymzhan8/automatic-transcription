...
TRAIN_SCRIPT = PROJECT_ROOT / "training" / "train_cnn.py"
OUTPUT_ROOT = PROJECT_ROOT / "output" / "ab_denoise_runs"
...
def main() -> None:
    ...
    parser.add_argument(
        "--variants",
        nargs="+",
        choices=("raw", "denoised", "vocals"),
        default=None,
        help="Train only these variants (default: all three)",
    )
    ...
    selected = args.variants or list(all_variants.keys())
    variants = {name: all_variants[name] for name in selected}
    ...
    # Always align against all available variants so row keys match across A/B arms,
    # even when training a subset (e.g. vocals-only on the 11 vocal pieces).
    present = {name: path for name, path in all_variants.items() if path.exists()}
    if not present:
        raise FileNotFoundError("No variant metadata found under output/cnn_dataset*/all/")
    aligned_variants = align_metadata_variants(present, ALIGNED_DIR)
    aligned_variants = {name: aligned_variants[name] for name in selected}
