...
def denoise_piece(raw_audio: Path, denoise_dir: Path) -> tuple[Path, Path]:
    """Return (denoised_path, vocals_path), using cached stems when present."""
    ...
    print(f"  running vocalprep on {raw_audio.name} ...")
    pipe = VocalPipeline(output_dir=denoise_dir)
    result = pipe.process(raw_audio)
    if result.denoised is None or not result.denoised.exists():
        raise FileNotFoundError(f"Denoise stage did not produce output for {raw_audio.name}")
    if result.vocals is None or not result.vocals.exists():
        raise FileNotFoundError(f"Separation stage did not produce vocals for {raw_audio.name}")
    return result.denoised, result.vocals
    ...
    for i, piece_id in enumerate(piece_ids, start=1):
        ...
        try:
            ...
            if args.skip_denoise:
                ...
                if not denoised_path.exists() or not vocals_path.exists():
                    print(f"  SKIP: cached stems missing in {piece_denoise_dir}")
                    continue
            else:
                denoised_path, vocals_path = denoise_piece(raw_audio, piece_denoise_dir)
            ...
            print(f"  exported denoised={len(metadata_denoised)}, vocals={len(metadata_vocals)}")
